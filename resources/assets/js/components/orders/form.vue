		...
		<form @submit.prevent>
			...
			<b-row>
				...
				<b-table
					...
					<template slot="cell(name)" slot-scope="data" class="inline-cart-products">
					    {{ data.item.product.name }}
					</template>
					<template slot="cell(variant)" slot-scope="data" class="inline-cart-products">
					    {{ data.item.productSKU.condition }}
					</template>
					<template slot="cell(quantity)" slot-scope="data" class="inline-cart-products">
						<quantity-control
							:showLabel="false"
					...
					<template slot="cell(basePrice)" slot-scope="data" class="inline-cart-products">
					...
					<template slot="cell(productTotal)" slot-scope="data">
						{{ data.value | currency }}
					</template>
		        </b-table>
		        ...
		        <b-col cols="6">
			        ...
			        <table class="table table-striped">
			        	<tr>
			        		<td><strong>{{ $t( 'order_subtotal' ) }}:</strong></td>
			        		<td>{{ subTotal | currency }}</td>
			        	</tr>
			        	<tr>
			        		<td><strong>{{ $t( 'order_discount' ) }}:</strong></td>
			        		<td>
			        			<editable-number-field
			...
			<b-row>
				...
				<b-col>
					...
					<b-form-select
						v-model="form.FK_deliveryMethod"
...
import QuantityControl from '@cbcAdmin/components/ui/quantity-control';
import EditableNumberField from '@cbcAdmin/components/ui/editable-number-field';
import { Form } from '@cbcAdmin/classes/form';
import NewCustomerSidebar from './new-customer-sidebar';
import addressFormFields from '@cbcAdmin/components/address/address-form-fields';
export default {
	...
	computed: {
		...
		...mapState({
			deliveryMethods : state => state.globalData.settings.deliveryMethods.map( m => { return { "value" : m.id, text : m.label } } )
		}),
		subTotal() {
	...
	methods: {
		...
		fetchProductSKUs() {
			var self = this;
			var assignSkus = function(){
				if( self.currentProduct ){
					var skus = self.$options.filters.denormalize( self.currentProduct.skus );
					if( skus.length ){
						self.productSKUs = skus;
					}
				}
			}
			if( !this.currentProduct.skus ){
				let includes = [
					"skus",
					"categories",
					"media"
				];
				this.getProduct( { id: this.currentProduct.id, profile: "admin", includes: includes.join(','), activeSKUsOnly : true } )
					.then( product => {
						self.$store.commit( 'updateProductState', product );
						assignSkus();
					})
					.catch( err => {
						console.error( err );
