...
const getters = {
	...
	productsListArray : state => state.productsList ? Vue.options.filters.denormalize( state.productsList ) : [],
	currentProductID  : state => state.currentProductID,
	currentProduct    : state => state.activeProduct || ( state.productsList.resultsMap ? state.productsList.resultsMap[ state.currentProductID ] : null ),
...
const actions = {
	...
	fetchProductConditions: ( context ) => new Promise( ( resolve, reject ) => {
		api().get.products.conditions()
            .then(XHR => {
				context.commit( "updateGlobalData", { "key" : "productConditions", value : XHR.data } );
                resolve();
            })
            .catch(err => {
                console.error(err);
                reject( "Error: An error occurred while fetching product conditions" );
            })
	} )
};
...
const mutations = {
	...
	updateProductState( state, product ){
		Vue.set( state.productsList.resultsMap, product.id, product );
	},
