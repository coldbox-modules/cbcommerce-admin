...
const actions = {
    ...
    deleteCategory: ( context, categoryId ) => new Promise( (resolve, reject) => {
		api().delete.categories.delete( categoryId )
			.then( ( XHR ) => {
				context.commit( "removeCategory", categoryId );
			} )
...
const mutations = {
	...
	removeCategory( state, categoryId ){
		let topLevelIndex = state.categories.results.indexOf( categoryId );
		if( topLevelIndex > -1 ){
			state.categories.results.splice( topLevelIndex, 1 );
			Vue.delete( state.categories.resultsMap, categoryId );
		} else {
			state.categories.results.forEach( function( id ){
				let category = state.categores.resultsMap[ id ];
				let childIndex = category.findIndex( child => child.id == categoryId );
				if( childIndex > -1 ){ state.categories.resultsMap[ id ].children.splice( childIndex, 1 ); }
			} );
		}
	}
};
