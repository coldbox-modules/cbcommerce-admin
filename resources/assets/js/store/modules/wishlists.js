...
const actions = {
	addItemToWishlist: ({ state, commit, getters, rootState }, { sku, wishlist } ) => {
		if( !rootState.globalData.authUser ) {
			return;
		}
	...
	getWishlists: ({  commit, rootState }, params ) => {
		if( !rootState.globalData.authUser ) return;
		return api()
