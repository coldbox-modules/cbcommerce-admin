...
export const updateCategoryMediaItem = api => ( data ) => {
  let endpoint = `/product-categories/${data.FK_category}/media/${data.id}`;
  delete data.originalData;
...
export const deleteCategoryMediaItem = api => (item) => api.delete( `/product-categories/${item.FK_category}/media/${item.id}` );
