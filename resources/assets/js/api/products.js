...
export const updateProduct = api => ( data ) => {
  ...
  data.includes = data.includes || "skus,categories,media";
  data.profile = "admin";
...
export const getProduct = api => ( id, params ) => {
  return api.get( '/products/' + id, { params : params || { includes : "skus,categories,media", profile : "admin" } } );
};
...
export const getSkuWithProduct = api => ( sku ) => {
  return api.get( '/skus/' + sku, { params : { includes : "product", profile: "admin" } } );
};
...
export const updateSKU = api => ( data ) => {
  ...
  delete payload.FK_product;
  if( payload.consignor && payload.consignor.id ){
	  payload.FK_consignor = payload.consignor.id;
  }
  delete payload.consignor;
  if( payload.subCondition && payload.subCondition.id ){
	  payload.FK_subCondition = payload.subCondition.id;
  }
  delete payload.subCondition;
  payload.profile = "admin";
  return api.put( `/skus/${data.id}`, JSON.stringify( payload ) );
}
...
export const updateProductMediaItem = api => (data) => {
  let endpoint = `/products/${data.FK_product}/media/${data.id}`;
  delete data.originalData;
  delete data.FK_product;
  return api.put( endpoint, JSON.stringify(data));
}
export const updateProductMediaItemField = api => (href, field, fieldValue) => {
  let payload = {};
  payload[field] = fieldValue;
  return api.patch( href, JSON.stringify(payload) );
};
export const deleteProductMediaItem = api => (item) => api.delete( `/products/${item.FK_product}/media/${item.id}` );
...
export const updateSKUMediaItem = api => (data) => {
  let endpoint = `/skus/${data.FK_sku}/media/${data.id}`;
  delete data.originalData;
  delete data.FK_sku;
  return api.put( endpoint, JSON.stringify(data) );
}
...
export const deleteSKUMediaItem = api => (item) => api.delete( '/skus/' + item.FK_sku + '/media/' + item.id );

export const getProductConditions = api => () => api.get( '/product-conditions' )
...
export const products = api => ( {
  get: {
    products: {
	  ...
	  conditions : getProductConditions( api )
    },
    skus : {
